...
class FlashcardsTableTableViewController: UITableViewController {

    // singleton from FlashcardsModel
...
//        cell.textLabel?.text = "Row \(indexPath.row)"
        ...
        if let card = model.flashcard(at: indexPath.row) {
            label.text = card.getQuestion()
            if let subtile = cell.detailTextLabel {
                subtile.text = card.getAnswer()
            }
        }
